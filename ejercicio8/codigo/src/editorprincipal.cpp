...
void EditorPrincipal::inicializarUI()
{
    ...
    QLabel *profileHeadline = new QLabel(
        "Software Engineering Student - Python, C++, HTML, CSS,Java",
        sidebarCard);
...
void EditorPrincipal::validarLinea(int blockNumber)
{
    ...
    const DocumentValidationResult documentResult =
        m_validadorActual->validarDocumento(m_editor->toPlainText());
    const QHash<int, QString> erroresPrevios = m_erroresPorLinea;
    m_erroresPorLinea = documentResult.errorsByLine;

    for (auto it = erroresPrevios.cbegin(); it != erroresPrevios.cend(); ++it) {
        if (!m_erroresPorLinea.contains(it.key())) {
    ...
    for (auto it = m_erroresPorLinea.cbegin(); it != m_erroresPorLinea.cend(); ++it) {
        const QString previousMessage = erroresPrevios.value(it.key());
        if (previousMessage != it.value()) {
    ...
    const bool lineaValida = !m_erroresPorLinea.contains(blockNumber);

    if (lineaValida) {
        if (blockNumber == m_editor->textCursor().blockNumber()) {
            QString message = QString("Linea %1 valida para %2.")
                .arg(blockNumber + 1)
                .arg(m_validadorActual->lenguaje());

            if (!documentResult.summaryMessage.isEmpty()) {
                message.append(' ');
                message.append(documentResult.summaryMessage);
            }

            if (!m_erroresPorLinea.isEmpty()) {
                message.append(QString(" Hay %1 error(es) en otra(s) linea(s).")
                               .arg(m_erroresPorLinea.size()));
            }

            actualizarPanelEstado(message);
        }
    } else {
        if (blockNumber == m_editor->textCursor().blockNumber()) {
            actualizarPanelEstado(QString("Linea %1: %2")
                                  .arg(blockNumber + 1)
                                  .arg(m_erroresPorLinea.value(blockNumber)),
                                  true);
        }
    }
